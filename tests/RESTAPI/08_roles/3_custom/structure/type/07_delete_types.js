...
* /v1/types endpoint
*/
describe('roles | custom > structure > type | delete type(s)', function() {
  ...
  it ('update permissions to delete types', function(done) {
    request
  ...
  it ('get the type deleted', function(done) {
    ...
    .expect(function(response) {
      assert(is.propertyCount(response.body, 2));
      assert(validator.equals(response.body.status, 'error'));
      assert(validator.equals(response.body.message, 'This type has not be found'));
    })
    .end(function(err, response) {
      if (err) {
        return done(err + ' | Response: ' + response.text);
