...
describe('roles | custom > structure > property | delete property(ies)', function () {
  ...
  it('update permissions to delete properties', function (done) {
    request
  ...
  it('get the property deleted', function (done) {
    request
      ...
      .expect(function (response) {
        assert(is.propertyCount(response.body, 2));
        assert(validator.equals(response.body.status, 'error'));
        assert(validator.equals(response.body.message, 'This item has not be found'));
      })
      .end(function (err, response) {
        if (err) {
          return done(err + ' | Response: ' + response.text);
