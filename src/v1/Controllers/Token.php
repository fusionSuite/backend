...
final class Token
{
  ...
  public function postToken(Request $request, Response $response, $args): Response
  {
    ...
    else if (\App\v1\Post::PostHasProperties($data, ['login']) === true
        && \App\v1\Post::PostHasProperties($data, ['password']) === true
        && trim($data->login !== '')
        && trim($data->password !== ''))
    {
      // Verify the account
      $user = \App\v1\Models\User::where([['login', $data->login]])->get()->makeHidden(\App\v1\Common::getFieldsToHide($model->getVisible(), ['id', 'login', 'password', 'jwtid', 'refreshtoken', 'firstname', 'lastname', 'displayname']))->makeVisible(["password", "jwtid"])->toArray();
      if (count($user) == 0)
      {
