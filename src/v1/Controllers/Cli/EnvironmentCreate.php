...
class EnvironmentCreate extends Command
{
  public function __construct()
  {
    ...
    $this
      ...
      ->option('-c --current', '[option] This will define this new environment as the current environment configuration')
      // Usage examples:
      ->usage(
        '<bold>  env:create</end> <comment> ## create an environment configuration - interactive mode<eol/>' .
        '<bold>  env:create</end> <comment>--name staging --type MariaDB --host 127.0.0.1 --username root --password secret ## create a MariaDB staging environment configuration<eol/>' .
        '<bold>  env:create</end> <comment>-n production -t PostgreSQL -h 192.168.20.10 -u root -p secret -P 5433 ## create a PostgreSQL production environment configuration<eol/>'
      );
  }
  ...
  public function interact(Interactor $io)
  {
    ...
    if (!$optionsCalled)
    {
      ...
      $this->set('name', $io->prompt('Enter the name of the environment configuration (examples: production, staging...)'));
