...
trait Read
{
  ...
  public function paramFilters($params, $items, $typeId = null)
  {
    ...
    {
      // manage pagination, we ignore them because managed in function manageParams()
      if (in_array($key, ['page', 'per_page']))
      {
        continue;
      }
      // manage filtering by name of the item
      if ($key == 'name')
      {
        if (is_array($value))
        {
          ...
          {
            $this->paramFilterName(null, $singleValue, $items, $typeId);
          }
        } else {
          $this->paramFilterName(null, $value, $items, $typeId);
        }
        continue;
      }
      // manage filtering by name and operator (in, contains...)
      if (str_starts_with($key, 'name_'))
      {
        ...
        if (is_array($value))
        {
          foreach ($value as $singleValue)
          {
            $this->paramFilterName($operator, $singleValue, $items, $typeId);
          }
        } else {
          $this->paramFilterName($operator, $value, $items, $typeId);
        }
        continue;
      }
      ...
      if (count($matches) != 2 and count($matches) != 4)
      {
        ...
        if (is_array($value))
        {
          foreach ($value as $singleValue)
          {
            $this->paramFilterProperty($matches, $singleValue, $items, $typeId);
          }
        } else {
          $this->paramFilterProperty($matches, $value, $items, $typeId);
        }
        continue;
      }
    }
  ...
  /**
   * Create the link header when have multiple pages (pagination)
   */
  ...
  /**
   * Manage filter on item name only
   */
  private function paramFilterName($operator, $value, $items, $typeId = null)
  {
    if (is_null($operator))
    {
      Query::fieldWhere($items, 'name', $value);
    } else {
      switch ($operator)
      {
        case 'in':
          $values = explode(',', $value);
          Query::fieldWherein($items, 'name', $values);
            break;

        case 'contains':
          Query::fieldWhereLike($items, 'name', '%' . $value . '%');
            break;
        ...
        case 'not':
          Query::fieldWherenot($items, 'name', $value);
            break;
  ...
  private function paramFilterProperty($matches, $value, $items, $typeId)
  {
    ...
    if (count($matches) == 2)
    {
      ...
      if (is_null($value))
      {
        ...
        Query::propertyWhere($items, $value, $property);
      }
      return;
    }
    if (count($matches) == 4)
    {
      ...
      {
        $this->paramFilterPropertySuffixTypeId($items, $property, $value, $matches);
      }
      elseif ($this->isValuetypeIsDatetime($property->valuetype))
      {
        $this->paramFilterPropertySuffixTypeDateTime($items, $property, $value, $matches);
      }
      elseif ($this->isValuetypeIsString($property->valuetype))
      {
        $this->paramFilterPropertySuffixTypeString($items, $property, $value, $matches);
      } else {
        $this->paramFilterPropertySuffixTypeNumber($items, $property, $value, $matches);
      }
    }
  }
  ...
  private function paramFilterPropertySuffixTypeNumber($items, $property, $value, $matches)
  {
    switch ($matches[3])
    {
      case 'in':
        ...
        Query::propertyWherein($items, $values, $property);
          break;

      case 'not':
        $value = $this->convertNumericValue($value, $property->valuetype);
        $this->searchPropertyValidation($property->valuetype, $value);
        Query::propertyWherenot($items, $value, $property);
          break;
      ...
      case 'less':
        ...
        $value = $this->convertNumericValue($value, $property->valuetype);
        $this->searchPropertyValidation($property->valuetype, $value);
        Query::propertyWhereWithOperator($items, $value, $property, '<');
          break;
      ...
      case 'not':
        ...
        elseif ($property->valuetype == 'itemlinks' || $property->valuetype == 'typelinks')
        {
          ...
          foreach ($values as $val)
          {
            ...
            Query::propertyWherenot($items, $val, $property);
          }
        }
          break;
  ...
  /**
   * Manage sorting params in request
   */
  ...
  /**
   * Get the first property of the iten type have the property id
   */
  ...
  private function getSpecialPropertyFilterIds($property, $value, $operator)
  {
    ...
    switch ($operator)
    {
      ...
      default:
        ...
        {
          case 'contains':
            Query::fieldWhereLike($items, $searchField, '%' . $value . '%');
              break;
  ...
  /**
   * return true if the valuetype is a "time" type, otherwise false
   */
  ...
  /**
   * return true if the valuetype is a string type, otherwise false
   */
